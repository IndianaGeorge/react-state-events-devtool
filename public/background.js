...
const clearTabStorage = async function (tabId) {
  ...
  if (root[tabId]) {
    delete root[tabId];
    // chrome.storage.session.remove([tabId]); // session storage is removed automatically
  }
}
...
chrome.runtime.onConnect.addListener(function (port) {
  ...
  port.onMessage.addListener(async function (msg) {
    if (!validate(msg,['action', 'tabId'])) {
      console.error('Got a request from panel with no action or no tabId');
